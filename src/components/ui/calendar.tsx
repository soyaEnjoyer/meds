...
function Calendar({
  ...
  const formattersProp: ComponentProps<typeof DayPicker>['formatters'] = useMemo(
    () => ({
  ...
  const classNamesProp: ComponentProps<typeof DayPicker>['classNames'] = useMemo(
    () => ({
      button_next: cn(
    ...
    [defaultClassNames, buttonVariant, classNames, captionLayout, props.showWeekNumber]
  );

  const componentsProp: ComponentProps<typeof DayPicker>['components'] = useMemo(
    () => ({
      ...
      Root: ({ className: innerClassName, rootRef, ...rest }) => (
        <div data-slot='calendar' ref={rootRef} className={cn(innerClassName)} {...rest} />
      ),
