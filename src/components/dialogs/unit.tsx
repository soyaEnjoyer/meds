import { useCallback } from 'react';

import { Dialog, DialogContent } from '@/components/ui/dialog';
import { UnitForm } from '@/forms/unit';
import { useDialog } from '@/hooks/dialog';

export function UnitDialog() {
  const [closeDialog, setDialog] = useDialog((state) => [state.actions.close, state.actions.set]);
  const dialogState = useDialog((state) => state.unit);

  const handleOpenChange = useCallback((open: boolean) => setDialog('unit', open), [setDialog]);

  const props = dialogState.id ? ({ id: dialogState.id, mode: 'edit' } as const) : ({ mode: 'add' } as const);

  const wrappedCloseDialog = useCallback(() => closeDialog('unit'), [closeDialog]);

  return (
    <Dialog open={dialogState.open} onOpenChange={handleOpenChange} disablePointerDismissal>
      <DialogContent>
        <UnitForm {...props} closeDialog={wrappedCloseDialog} />
      </DialogContent>
    </Dialog>
  );
}
